...
// Helper function to parse date string and normalize to day start
const parseDateString = (dateString: string): Date => {
  // Parse YYYY-MM-DD format and ensure it's treated as local date
  const [year, month, day] = dateString.split('-').map(Number);
  const date = new Date(year, month - 1, day); // month is 0-indexed
  return normalizeDateToDayStart(date);
};
